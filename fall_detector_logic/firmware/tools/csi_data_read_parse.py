    ...
    while True:
        ...
        if count ==0:
            ...
            if csi_data_len == 106:
                ...
                # 52 Valid Subcarriers for HT20 (26 left, 26 right)
                colors = generate_subcarrier_colors((0,25), (26,51), None, 52)
        ...
        if csi_data_len == 128:
            # HT20: Extract only the 52 valid subcarriers (indices 6-31 and 33-58)
            valid_indices = list(range(6, 32)) + list(range(33, 59))
            for idx, valid_idx in enumerate(valid_indices):
                if valid_idx * 2 + 1 < len(csi_raw_data):
                    csi_data_complex[-1][idx] = complex(csi_raw_data[valid_idx * 2 + 1],
                                                        csi_raw_data[valid_idx * 2])
        else:
            for i in range(min(csi_data_len // 2, CSI_DATA_COLUMNS)):
