...
static void wifi_init()
{
    ...
    // NOTE: MAC override removed — RX must keep its own MAC to avoid
    // filtering its own reflected management traffic as valid CSI data.
}
...
static void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info)
{
    ...
    // Reject non-HT20 frames: expect exactly 128 bytes (64 I/Q pairs)
    if (info->len != 128) {
        return;
    }
    ...
    len += snprintf(serial_buf + len, sizeof(serial_buf) - len, "CSI_DATA,%lu," MACSTR ",%d,%d,%d,%d,%d,%d,%lu,%d,%d",
    ...
    len += snprintf(serial_buf + len, sizeof(serial_buf) - len, "CSI_DATA,%lu," MACSTR ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%lu,%d,%d,%d",
               (unsigned long)rx_id, MAC2STR(info->mac), rx_ctrl->rssi, rx_ctrl->rate, rx_ctrl->sig_mode,
               rx_ctrl->mcs, rx_ctrl->cwb, rx_ctrl->smoothing, rx_ctrl->not_sounding,
               rx_ctrl->aggregation, rx_ctrl->stbc, rx_ctrl->fec_coding, rx_ctrl->sgi,
               rx_ctrl->noise_floor, rx_ctrl->ampdu_cnt, rx_ctrl->channel, rx_ctrl->secondary_channel,
               (unsigned long)rx_ctrl->timestamp, rx_ctrl->ant, rx_ctrl->sig_len, rx_ctrl->rx_state);
    ...
    len += snprintf(serial_buf + len, sizeof(serial_buf) - len, ",%d,%d,\"[%d", (info->len - 2) / 2, info->first_word_invalid, (int16_t)(compensate_gain * csi));
    for (int i = 2; i < (info->len - 2); i += 2) {
        csi = ((int16_t)(((((uint16_t)info->buf[i + 1]) << 8) | info->buf[i]) << 4) >> 4);
        len += snprintf(serial_buf + len, sizeof(serial_buf) - len, ",%d", (int16_t)(compensate_gain * csi));
    }
#else
    len += snprintf(serial_buf + len, sizeof(serial_buf) - len, ",%d,%d,\"[%d", info->len, info->first_word_invalid, (int16_t)(compensate_gain * info->buf[0]));
    for (int i = 1; i < info->len; i++) {
        len += snprintf(serial_buf + len, sizeof(serial_buf) - len, ",%d", (int16_t)(compensate_gain * info->buf[i]));
    }
#endif
    len += snprintf(serial_buf + len, sizeof(serial_buf) - len, "]\"");
